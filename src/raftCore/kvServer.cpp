...
void KvServer::GetCommandFromRaft(ApplyMsg message) {
  ...
  if (!ifRequestDuplicate(op.ClientId, op.RequestId)) {
    ...
    if (op.Operation == "TxnPrewrite" || op.Operation == "TxnAcquirePessimisticLock") {
      TxnOpPayload payload;
      payload.parseFromString(op.Value);
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) {
  ...
  m_engineType = KVEngineFactory::ResolveFromEnvOrDefault();
  const std::string dbPath = "run_data/rocksdb_node_" + std::to_string(m_me);
  m_kvEngine = KVEngineFactory::Create(m_engineType, dbPath);
...
void KvServer::TxnPrewrite(google::protobuf::RpcController *controller, const ::raftKVRpcProctoc::TxnPrewriteArgs *request,
  ...
  payload.primaryKey = request->primarykey();
  payload.startTs = request->startts();
  payload.ttlMs = request->ttlms();
  payload.isDelete = request->isdelete();
...
void KvServer::TxnCommit(google::protobuf::RpcController *controller, const ::raftKVRpcProctoc::TxnCommitArgs *request,
  ...
  TxnOpPayload payload;
  payload.startTs = request->startts();
  payload.commitTs = request->committs();
