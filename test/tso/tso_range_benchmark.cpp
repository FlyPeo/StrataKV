/*
 * 测试目标：比较 TSO 不同时间戳预留区间大小对 Raft proposal 次数、吞吐和延迟的影响。
 * 测试策略：分别启动隔离的三成员 TSO 集群，完成 Leader 预热后，以相同客户端数和
 *           请求数并发取号，只改变 baseline/optimized range size。
 * 测试规模：默认运行 range=1 和 range=4096 两组；每组 128 次预热、8 个客户端并发
 *           完成 2,048 次正式请求，共 4,096 次正式取号。
 * 验证内容：确认时间戳全局唯一、Leader 在测量窗口内稳定、请求全部完成，并校验
 *           reservation/proposal 统计后输出两组吞吐与 P50/P95/P99。
 */
#include <arpa/inet.h>
#include <fcntl.h>
#include <signal.h>
