/*
 * 测试目标：验证本地持久化 HLC 时间戳生成器在时钟异常、边界和重启情况下仍保持单调。
 * 测试策略：注入可控物理时钟并使用临时状态文件，覆盖同毫秒递增、时钟回拨、逻辑位
 *           溢出、旧十进制状态迁移、进程重建以及未知版本输入。
 * 测试规模：固定 5 个顶层场景，包含 compose/extract、同毫秒与回拨、逻辑位溢出、
 *           legacy 迁移重启和未知版本拒绝。
 * 验证内容：确认 physical/logical 编解码正确，任何情况下都不重复或倒退，迁移后的持久
 *           high-water 不低于已发时间戳，且无法识别的状态版本必须失败关闭。
 */
#include <filesystem>
#include <fstream>
#include <iostream>
