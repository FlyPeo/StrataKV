/*
 * 测试目标：验证 NodeServer 对在线分裂结果的物化：父 Region 原地收缩、子 Region 上线并注册进调度器。
 * 测试策略：在 mkdtemp 临时目录构建含 2 个 serving Region 的 NodeServer，模拟父 Region 已应用
 *           AdminSplit（描述符收缩、epoch 升版）后调用 MaterializeSplitChild，检查 registry、
 *           调度器与 peer 句柄，再重放同一物化验证幂等。
 * 测试规模：1 个分裂物化场景 + 1 次重放（共 2 次调用），registry 从 2 个 serving 变为 3 个。
 * 验证内容：物化出的子 Region 立即为 Serving 并注册进 NodeTxnScheduler，父 Region 句柄保持同一
 *           peer 实例（原地更新而非重建），registry serving 计数变为 3；丢失 ack 后的重放是幂等
 *           no-op，不产生重复子 Region。
 */
#include <filesystem>
#include <iostream>
