/*
 * 测试目标：验证客户端 RegionCache 的半开区间路由、原子替换、并发读安全与刷新合并。
 * 测试策略：纯内存 RegionCache 上以 2 Region 基线驱动 LookupKey/LookupRegion/ReplaceInterval，
 *           用 8 线程并发读压发布中的路由表，再以阻塞式 refresh 回调验证合并、独立截止时间与失败恢复。
 * 测试规模：3 组场景：边界与替换（含 gap/overlap 拒绝）、8 并发读者、刷新合并与等待者超时。
 * 验证内容：空键路由左、half-open 分裂键路由右，新 revision 原子发布两个子 Region 且旧响应被
 *           丢弃、gap/overlap 刷新抛 invalid_argument；读者永远看不到部分表；等价 stale-range miss
 *           共享一次元数据拉取，等待者按自己的 deadline 超时，刷新失败释放槽位且成功结果仍可用。
 */
#include <atomic>
#include <chrono>
#include <iostream>
