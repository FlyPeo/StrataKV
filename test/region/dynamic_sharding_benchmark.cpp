/*
 * 测试目标：衡量在线动态分裂对真实客户端流量的性能与韧性影响，分裂期间必须零丢弃请求、零丢失写入。
 * 测试策略：面向运行中的集群（默认 3 节点 Metadata 与 TSO）用原生 SDK 多线程压测，按 100ms 窗口
 *           采样 QPS/延迟，经 admin 工具在键 "d" 处触发 Region 100 在线分裂，对比分裂前/中/后三阶段。
 * 测试规模：默认 8 个 worker，pre/in/post 三阶段各 5/6/5 秒，读写各半，全量审计三阶段写入的每个键。
 * 验证内容：分裂瞬间 QPS 瞬时跌幅、P99 尖峰与恢复时长，EPOCH_NOT_MATCH 只体现为 SDK 透明刷新，
 *           分裂后拓扑恰好多出一个子 Region，且跨分裂边界写入的全部键可读回、值与写入时一致（零丢失零损坏）。
 */
