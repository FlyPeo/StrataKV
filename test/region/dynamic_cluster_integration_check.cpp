/*
 * 测试目标：验证动态拓扑集群端到端——控制面/TSO/存储节点 bootstrap、事务路由与故障阶段行为。
 * 测试策略：拉起真实 meta×3 + tso×3 + node×3 动态集群,SDK 直连驱动单键与跨 Region 事务,并注入故障阶段。
 * 测试规模：3 节点全拓扑,含在线分裂与路由刷新窗口。
 * 验证内容：注册表原子发布、分裂后客户端透明改道、事务身份跨刷新保持、全程零请求丢弃。
 */
#include <arpa/inet.h>
#include <fcntl.h>
