/*
 * 测试目标：验证 RegionRequestSender 的路由重试语义：传输失败、NotLeader、EpochNotMatch、
 *           RegionNotFound、存储失败、响应丢失、deadline 耗尽与批次重分组。
 * 测试策略：完全脚本化的 kvServerRpc stub（无 socket/Raft/RocksDB）按步骤注入错误与 leader hint，
 *           配合脚本化元数据刷新，观察每次尝试落到哪个 peer、client/request 身份是否跨重试保持。
 * 测试规模：8 个场景外加迁移 NotLeader 特例，重试预算最多 64 次、最短 deadline 120ms。
 * 验证内容：健康路由一次成功；传输失败换 peer 重试且不失效当前缓存路由；NotLeader 精确跟随
 *           hint 重试且身份不变，指向新迁移 peer 的 hint 被当作拓扑证据触发刷新；epoch/Region 缺失
 *           刷新元数据并重路由；存储错误不重试；永久丢响应耗尽 deadline 并计 exhaustion；分裂批次要求 regroup。
 */
#include <chrono>
#include <iostream>
#include <memory>
