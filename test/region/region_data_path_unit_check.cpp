/*
 * 测试目标：验证拓扑感知数据通路：ShardRouter 跟随缓存分裂重建分片，RaftMvccStorage 经 sender
 *           携带 header 重试并在分裂时请求重分组。
 * 测试策略：脚本化 kvServerRpc stub 记录每次上线请求的 RegionRequestHeader 并按步骤应答
 *           NotLeader/Storage/EpochNotMatch，配合脚本化 RegionCache 刷新，进程内驱动 BatchPrewrite 全链路。
 * 测试规模：5 个场景：路由跟随分裂、header/幂等身份携带、NotLeader 重试、确定性存储错误、
 *           分裂批次重分组。
 * 验证内容：分裂后受影响键迁移到新 Region 的惰性新建 storage 而未变 Region 复用实例；批次携带
 *           解析出的 Region id/epoch 与 client/request 幂等身份，NotLeader 重试复用同一 request id，
 *           存储错误一次直达调用方，EpochNotMatch 批次刷新拓扑并置 LastRegroupRequired 而不虚报成功。
 */
#include <chrono>
#include <iostream>
#include <memory>
...
int main() {
  try {
    ...
    // 4b. Overload is transient: it must surface as ResultUnknown so the
    // caller keeps retrying within budget; mapping it to a definitive
    // StorageError stalled secondary commits during the B1 fault window.
    {
      auto fixture =
          MakeStorage({{stratakv::region::REGION_ERROR_SERVER_OVERLOADED, 0, "9"}});
      const auto status = fixture.storage->BatchPrewrite(
          {{"apple", "v", false, false}}, "apple", 100, 3000);
      Require(status != TxnStatus::StorageError,
              "overload must not be promoted to a definitive storage error");
      Require(status == TxnStatus::ResultUnknown, "overload must stay result-unknown");
    }
