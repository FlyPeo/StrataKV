...
def full_report(
    ...
    # w32 dropped 2026-09-19: 32-way concurrency saturates the co-located
    # WSL baseline (scheduler queue-full rejections at F-w32), so it is not
    # part of the reported matrix; revisit on dedicated hardware.
    points = [f"a1-{wl}-w{w}" for wl in "ABCF" for w in (1, 4, 8, 16)]
...
def main() -> int:
    ...
    if full_family:
        # w32 dropped 2026-09-19 (saturates the co-located WSL baseline; see
        # the A1 note above) — required set matches the measured matrix.
        points = [f"a1-{wl}-w{w}" for wl in "ABCF" for w in (1, 4, 8, 16)]
        points += [f"a1-zipfian-{wl}-w8" for wl in "AB"]
        points += [f"a1-direct-{wl}-w8" for wl in "AC"]
        points += [f"a2-cross-{v}" for v in (0, 15, 100)]
