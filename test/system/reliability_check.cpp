 ...
 * 测试策略：通过 Gateway 并发写入三键事务，记录每次提交确认与延迟，再并发读取校验；
 *           支持 verify-only 对脚本重启或故障注入后的同一批数据再次验证。
 * 测试规模：默认 8 个 worker 执行 1,000 个事务，即写入并校验 3,000 个键；每次事务或
 *           读取最多重试 12 次，规模与重试预算均可通过参数调整。
...
std::string Begin(const Endpoint& endpoint, const Options& options) {
  const HttpResponse response =
      Request(endpoint, options.timeoutMs, "POST", "/v1/transactions", "{\"lockTtlMs\":120000}");
