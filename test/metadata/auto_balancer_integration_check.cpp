/*
 * 测试目标：验证自动均衡 operator 的端到端生命周期——派发、阶段推进、leader 重启后 reconcile。
 * 测试策略：三成员元数据集群 + 注入式操作协调器,按持久化 operator 状态机驱动并中途换主。
 * 测试规模：多 operator 串行/并行场景,含取消与重放路径。
 * 验证内容：不可逆点前可取消、重启后从持久化阶段续跑不重复派发、拒绝路径可解释。
 */
#include <chrono>
#include <filesystem>
#include <iostream>
