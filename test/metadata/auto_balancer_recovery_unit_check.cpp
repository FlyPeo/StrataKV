/*
 * 测试目标：验证调度操作协调器在 metadata Leader 切换与算子取消场景下不丢工作、不重复执行。
 * 测试策略：用内存 MetadataStateMachine 加可脚本化步骤钩子的 TopologyOperationCoordinator，
 *           模拟旧 Leader 停摆、新 Leader 从持久化的 pending/waiting 阶段接手 move-peer 算子并继续推进。
 * 测试规模：5 个场景：旧 Leader 不派发、pending 阶段交接、waiting 阶段交接、晋升前取消、
 *           Auto-Balancer 生命周期。
 * 验证内容：former Leader 不派发任何目标准备或 conf-change，新 Leader 补完剩余步骤并完成 learner
 *           晋升，waiting（不可逆）后取消返回 CONFLICT，晋升前取消安全回滚到原 quorum，非 Leader
 *           的评估被跳过且不提交命令，balancer 可受控启停。
 */
#include <chrono>
#include <iostream>
#include <memory>
