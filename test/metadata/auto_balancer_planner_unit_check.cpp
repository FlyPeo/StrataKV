/*
 * 测试目标：验证 Auto-Balancer 调度规划器的副本迁移/修复决策、预算约束与分裂证据判定。
 * 测试策略：在纯内存 MetadataView（2 个 Region、4 个 Store、脚本化 StoreHeartbeat）上直接调用
 *           planner，精确控制磁盘水位、请求热度、冷却窗口与操作预算，观察产出的调度计划。
 * 测试规模：5 个决策场景：健康均衡、无目标修复、预算/阈值/冷却、连续分裂证据窗口、拓扑避让。
 * 验证内容：均衡计划 reason 为 Balance 且序列化字节稳定可重放，无新目标 Store 时拒绝迁移并给出
 *           可解释原因，超预算/低于不均衡阈值/冷却期内不出计划，分裂需连续 2 个证据窗口且计数可重置，
 *           存在 move-peer 拓扑工作时推迟同一 Region 的分裂。
 */
#include <iostream>
#include <memory>
#include <stdexcept>
