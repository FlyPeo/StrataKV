/*
 * 测试目标：验证三成员元数据集群端到端行为——bootstrap、failover、quorum 丢失、快照压缩与全进程重启。
 * 测试策略：启动三个真实 stratakv-meta 子进程,通过 MetadataClient 混合并发读写并注入成员故障。
 * 测试规模：3 成员多轮 Mutate/Lookup/Scan,含 leader 重启与少数派隔离窗口。
 * 验证内容：读走 ReadIndex 线性化视图、mutationId 幂等、revision 单调、快照截断被原子拒绝,故障绝不发布部分视图。
 */
#include <arpa/inet.h>
#include <fcntl.h>
#include <signal.h>
