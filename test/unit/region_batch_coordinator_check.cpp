/*
 * 测试目标：验证事务协调器按 Region 批处理多键 2PC，并保持 Primary-First 与兼容降级语义。
 * 测试策略：使用记录调用次数和并发度的伪 RegionStorage，提交六键三 Region 事务，分别
 *           模拟全成功、一个 Region prewrite 失败以及旧协议版本。
 * 测试规模：固定 3 个顶层场景，每个场景包含 6 个键并均匀分布到 3 个 Region，
 *           分别统计批量/逐键 prewrite、commit 和 rollback 调用次数。
 * 验证内容：确认每个 Region 只收到一次批调用且可并行，Primary 先提交、失败时所有潜在
 *           prewrite 都被回滚，协议 v2 则完整降级为逐键 RPC。
 */
#include <atomic>
#include <chrono>
#include <memory>
