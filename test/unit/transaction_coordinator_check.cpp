/*
 * 测试目标：验证分布式事务协调器在悲观锁、Primary-First 2PC 和异常恢复中的状态机语义。
 * 测试策略：使用内存引擎、可控 TSO 与故障注入 RegionStorage，构造锁冲突、写偏斜、
 *           deadline、未知提交结果、远程失败和协调器崩溃后锁清理等场景。
 * 测试规模：固定 6 个顶层场景，覆盖事务状态与清理、Primary 稳定性、写偏斜、超时与
 *           未知结果、远程失败分类以及协调器崩溃恢复。
 * 验证内容：确认事务正确进入 AbortOnly/ResultUnknown 等状态，Primary 保持稳定，部分操作
 *           被清理，受保护的写偏斜被阻止，超时分类及过期锁恢复不破坏已提交数据。
 */
#include <algorithm>
#include <cstdint>
#include <iostream>
