...
Automatic scheduling SHALL be disabled by default and SHALL run only in replicated dynamic metadata mode after an operator enables it. Exactly the current metadata leader SHALL evaluate and dispatch automatic operators. A follower or a former leader MUST NOT dispatch work, and a metadata quorum loss MUST stop new scheduling decisions while leaving committed topology unchanged.
...
- **WHEN** a node uses legacy static topology or automatic scheduling remains disabled
