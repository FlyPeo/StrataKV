...
- [ ] 4.6 复跑已经消除正常路径字符串构造的严格 Pulsar/Photon context A/B，并增加池关闭/开启的 Pulsar transfer 对照；验证：相同编译参数、栈大小、guard 与 CPU 下至少五轮，Pulsar 纯 transfer 中位数回退不超过 3%，Photon 对比表标明版本和 pool 状态；只追加复用相关数据，保留 `test/benchmarks/fiber_context_ab_benchmark.cpp` 与既有工业对比中的用户改动。
